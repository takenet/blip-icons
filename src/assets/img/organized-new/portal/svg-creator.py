      ...
      if svgFile.endswith(".svg"):
        ...
        with open(svgFilePath) as svg:
          ...
          if width is not None and len(width.group(0)) >= 1:
            ...
            valueSearch = re.search("([0-9]+){1}", widthString)
            widthValue = valueSearch.group(0)
            print(widthValue)
          else:
            widthString = ""
          ...
          if height is not None and len(height.group(0)) >= 1:
            ...
            valueSearch = re.search("([0-9]+){1}", heightString)
            heightValue = valueSearch.group(0)
            print(heightValue)
          else:
            heightString = ""
          ...
          elif widthValue is not None and heightValue is not None:
            viewboxString = ''.join([' viewBox="0 0 ', widthValue, ' ', heightValue, '"'])
          else:
            viewboxString = ""
          ...
          path = ''.join(['\t\t<symbol id="', fileName, '"', viewboxString, ">", svgPath, '</symbol>\n'])
          defs.write(path)
