...
with open(sys.argv[1], 'r') as r:
  ...
  for i in range(0, len(glyphs)):
    ...
    if filename:
	    ...
	    filename = str(i + 1).rjust(3, '0')
    with open(filename + '.svg', 'w') as w:
      w.write('<?xml version="1.0" standalone="no"?>\n')
      w.write('<svg width="1024px" height="1024px" version="1.1" xmlns="http://www.w3.org/2000/svg">\n')
      # replace 'glyph' with 'path' and flip vertically
      w.write(glyphs[i].replace('<glyph', '<path transform="scale(1, -1) translate(0, -1024)"') + '\n')
      w.write('</svg>')